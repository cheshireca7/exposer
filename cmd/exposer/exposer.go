...
func main() {
	...
	result := func(result sources.Result) {
		...
		// Check if IP was already stored, otherwise update ports
		ip, _ := data["ip"].(string)
	...
	// Monitor uncover query 
	for {
		if err := u.ExecuteWithCallback(context.TODO(), result); err != nil {
			panic(err)
		}
		time.Sleep(time.Duration(opt.updateTime) * time.Second)
	}
